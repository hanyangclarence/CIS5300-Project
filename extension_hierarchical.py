        ...
        for child in root:
            if child.tag == 'ABSTRACT':
                ...
                if 'intro' in section_title:
                    ...
                    section_text = '\n'.join([sub_child.text for sub_child in child if sub_child.text is not None])
...
def calculate_section_word_limits(sections, total_word_limit=150):
...
def summarize(model, TOKENIZER, text_input):
...
def summarize_document(model, tokenizer, sections, text_len, sum_len, total_summary_words=150):
    summarized_sections = []
    for title, text in sections.items():
        summarized_section = summarize_segment(model, tokenizer, text, text_len, sum_len)
        summarized_sections.append(summarized_section)

    combined_summary = ' '.join(summarized_sections)

    if len(combined_summary.split()) > total_summary_words:
        final_summary = summarize_segment(model, tokenizer, combined_summary, text_len, total_summary_words)
    else:
...
if __name__ == "__main__":
    ...
    for i in tqdm(range(len(test_df)), desc='Infer'):
        ...
        if len(text.split()) <= AVG_GT_WORDS:
            pred_sum = text
        else:
            text_sentences = re.split(r'\.\s', text)
            prefix_pred_sum = "\n".join(text_sentences[:5])
            sections = test_df['Sections'][i]
            hierarchical_sum = summarize_document(model, TOKENIZER, sections, TEXT_LEN, SUM_LEN, AVG_GT_WORDS)
            pred_sum = prefix_pred_sum + "\n" + hierarchical_sum
